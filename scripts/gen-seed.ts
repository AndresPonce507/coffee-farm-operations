...
import { plotGeometries, reserveZone } from "@/lib/geo/seed-geometry";
...
/** A GeoJSON value emitted as a `'…'::jsonb` literal. */
class JsonbLit {
  constructor(public readonly value: unknown) {}
}
const jsonb = (v: unknown) => new JsonbLit(v);

type Val = string | number | boolean | null | JsonbLit;
const lit = (v: Val): string => {
  if (v === null) return "null";
  if (v instanceof JsonbLit) {
    return `'${JSON.stringify(v.value).replace(/'/g, "''")}'::jsonb`;
  }
...
// Placeholder GeoJSON geometry, keyed by plot id (single source of truth:
// src/lib/geo/seed-geometry.ts — same math the migration's fix-up uses).
const geomById = new Map(plotGeometries().map((g) => [g.id, g]));
...
blocks.push(
  insert(
    ...
    plots.map((p, i) => {
      ...
      if (!g) throw new Error(`gen-seed: no placeholder geometry for "${p.id}"`);
      return [
        p.id, i, p.name, p.block, p.variety, p.areaHa, p.altitudeMasl, p.trees,
        p.shadePct, p.establishedYear, p.status, p.lastInspected,
        p.expectedYieldKg, p.harvestedKg, jsonb(g.geom), jsonb(g.centroid),
      ];
    }),
  ),
);
...
// reserve_zones — the protected quetzal reserve (PLACEHOLDER outline).
const reserve = reserveZone();
blocks.push(
  insert(
    "reserve_zones",
    ["id", "name", "kind", "geom", "area_ha", "notes"],
    [
      [
        reserve.id, reserve.name, reserve.kind, jsonb(reserve.geom),
        reserve.areaHa, reserve.notes,
      ],
    ],
  ),
);
