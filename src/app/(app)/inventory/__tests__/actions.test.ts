...
/** A Supabase-client stand-in: `.rpc()` for grading, `.from().insert()` for reserve. */
...
describe("gradeGreenLotAction", () => {
  it("grades a lot, revalidates /inventory, and returns the green code", async () => {
    const { client, rpc } = makeClient({
      rpc: { data: "JC-564-G", error: null },
    });
    ...
    if (state.status === "success") {
      expect(state.greenLotCode).toBe("JC-564-G");
    }
  ...
  it("surfaces a labelled error when the conservation trigger rejects over-routing", async () => {
    ...
    if (state.status === "error") {
      expect(state.message).toContain("mass conservation");
    }
    expect(revalidatePathMock).not.toHaveBeenCalled();
  });
});
